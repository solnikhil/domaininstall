...
- reachable-path search for `.env`, npm configuration, private keys,
  certificates, and common credential-file extensions
...
`git fsck` reported only unreachable dangling tree objects — local
garbage-collection candidates that are not transferred by an ordinary push.
